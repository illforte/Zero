import { mapGoogleLabelColor, mapToGoogleLabelColor } from './google-label-color-map.js';
import type { IOutgoingMessage, Label } from '../types.js';
import type { MailManager, ManagerConfig, ParsedMessage, CreateDraftData, ParsedDraft } from './types.js';
...
// ─── Inlined from apps/server/src/lib/email-utils.ts ────────────────────────

// Simple inline implementations that don't rely on the email-addresses package
...
export const parseFrom = (fromHeader: string): ParsedSender => {
  if (!fromHeader) return FALLBACK_SENDER;

  // Match: "Display Name <email@example.com>" or just "email@example.com"
  ...
  // Plain email address
  const emailMatch = fromHeader.match(/[\w.+-]+@[\w.-]+\.[a-zA-Z]{2,}/);
  if (emailMatch) {
    return { name: fromHeader.trim(), email: emailMatch[0] };
  }

  return { name: fromHeader.trim(), email: FALLBACK_SENDER.email };
};

export const parseAddressList = (header: string): ParsedSender[] => {
  if (!header) return [];
  ...
  for (const part of parts) {
    ...
    const parsed = parseFrom(trimmed);
    if (parsed.email && parsed.email !== FALLBACK_SENDER.email) {
      results.push(parsed);
    }
  }

  return results.length > 0 ? results : [FALLBACK_SENDER];
};
...
// ─── Inlined from apps/server/src/lib/utils.ts ──────────────────────────────
...
const cleanSearchValue = (q: string): string => {
...
// ─── Inlined from apps/server/src/lib/driver/google-label-color-map.ts ──────
// (already imported directly — no inlining needed here)

// ─── Replacement for sanitizeTipTapHtml ─────────────────────────────────────
// The original sanitizeTipTapHtml extracts inline images and sanitizes HTML.
// Here we use sanitize-html directly without inline image extraction.

interface SanitizeResult {
  html: string;
  inlineImages: Array<{ cid: string; mimeType: string; data: string }>;
}

const sanitizeTipTapHtml = async (html: string): Promise<SanitizeResult> => {
  const sanitized = sanitizeHtml(html, {
    allowedTags: sanitizeHtml.defaults.allowedTags.concat([
      'img', 'figure', 'figcaption', 'picture', 'source',
      'table', 'thead', 'tbody', 'tr', 'th', 'td',
      'h1', 'h2', 'h3', 'h4', 'h5', 'h6',
      'blockquote', 'pre', 'code', 's', 'u',
      'details', 'summary',
    ]),
    allowedAttributes: {
  ...
  return { html: sanitized, inlineImages: [] };
};
...
export class GoogleMailManager implements MailManager {
  ...
  public getUserInfo() {
    return this.withErrorHandler(
      ...
      async () => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const res = await people({ version: 'v1', auth: this.auth as any }).people.get({
  ...
  public count() {
    return this.withErrorHandler(
      ...
      async () => {
        ...
        const [userLabelsResult, archiveResult] = await Promise.allSettled([
        ...
        if (userLabelsResult.status === 'rejected') {
          return [];
        }

        const labels = userLabelsResult.value.data.labels || [];
        if (labels.length === 0) {
          return [];
        }
        ...
        if (archiveResult.status === 'fulfilled') {
          mapped.push({
            label: 'archive',
            count: Number(archiveResult.value.data.resultSizeEstimate ?? 0),
          });
        }
  ...
  private getQuotaUser() {
    ...
    return this.config.auth?.email ? `${this.config.auth.email}-${nodeEnv}` : undefined;
  }
    ...
    return this.withErrorHandler(
      ...
      async () => {
        ...
        return {
          ...
          messages,
          latest: [...messages].reverse().find((e: ParsedMessage) => e.isDraft !== true),
          hasUnread,
  ...
  public getDraft(draftId: string): Promise<ParsedDraft> {
    return this.withErrorHandler(
      'getDraft',
      async () => {
  ...
  > {
    ...
    const to = toHeaders.flatMap((to) => parseAddressList(to as string));
    ...
    const cc =
      ccHeaders.length > 0
        ? ccHeaders
            .filter((header) => (header as string).trim().length > 0)
            .flatMap((header) => parseAddressList(header as string))
        : null;
  ...
  private async parseDraft(draft: gmail_v1.Schema$Draft): Promise<ParsedDraft | null> {
  ...
  private async withErrorHandler<T>(
    ...
    try {
      ...
      throw new Error(`[Gmail:${operation}] ${err.message || 'Unknown error'}`);
    }
  }
